...
class Command(BaseCommand):
    help = "Import Master CR database from Excel utilizing Copy-on-Write (CoW) architecture"
    ...
    def handle(self, *args, **options):
        ...
        # Wrap in an atomic transaction so partial failures rollback completely
        with transaction.atomic():
            for row in df.to_dict(orient="records"):
                ...
                # Retrieve the currently active record for this CR
                active_record = MasterCRDatabase.objects.filter(
                    cr_no=cr_number, 
                    is_active=True
                ).first()

                if active_record:
                    # Change Detection: Check if any Excel column differs from the DB record
                    changed = False
                    for key, value in row.items():
                        record_val = getattr(active_record, key, None)
                        
                        # Note: You may need to handle pd.Timestamp vs datetime.date comparisons here 
                        # depending on your exact Django model field types.
                        if record_val != value:
                            changed = True
                            break
                    
                    # Only execute the CoW insertion if data has actually been modified
                    if changed:
                        # 1. Deactivate the old record (bypassing save() to prevent recursion)
                        MasterCRDatabase.objects.filter(pk=active_record.pk).update(is_active=False)
                        
                        # 2. Insert the new CoW version
                        MasterCRDatabase.objects.create(
                            **row,
                            is_active=True,
                            version=active_record.version + 1,
                            parent_record_id=active_record.pk
                        )
                        updated_cow += 1
                else:
                    # Brand new record, insert as version 1
                    MasterCRDatabase.objects.create(
                        **row,
                        is_active=True,
                        version=1
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Import complete: {created} new records, {updated_cow} updated (CoW).")
        )
