...
def start_task(request, task_id):
    ...
    module_folder = Path(settings.BASE_DIR) / "dashboard" / "task_modules" / task_name_slug
    if not module_folder.exists():
        msg = f"Task module folder does not exist for '{task['name']}'."
    ...
    # GLOBAL_LOGS.append(
    # f"task_module_path value: {task_module_path!r}, type: {type(task_module_path).__name__}, task_id={task_id} ---- {_timestamp()}")

    # GLOBAL_LOGS.append(
    # f"start_task called with task_id={task_id} (type: {type(task_id).__name__}) ---- {_timestamp()}")
    ...
    if not hasattr(task_module, "run_task"):
        msg = f"Module not found or run_task() missing for '{task['name']}'."
        GLOBAL_LOGS.append(f"{msg} ---- {_timestamp()}")
        return JsonResponse({"ok": False, "message": msg}, status=404)
    ...
    try:
        result = task_module.run_task(request=request, task=task, runtime=runtime, GLOBAL_LOGS = GLOBAL_LOGS, timestamp_fn=_timestamp)
        ...
        CURRENT_RUNNING_TASK = result.get(
            "message",
        ...
        return JsonResponse({
            "ok": True,
            "task_id": task_id,
            "message": CURRENT_RUNNING_TASK,
            "status": runtime["status"],
            "download_ready": runtime["download_ready"],
            "download_url": f"/download/task/{task_id}/" if runtime["download_ready"] else "",
            "counts": {
                "total_crs": runtime["total_crs"],
                "north_crs": runtime["north_crs"],
                "west_crs": runtime["west_crs"],
                "east_crs": runtime["east_crs"],
                "south_crs": runtime["south_crs"],
            }
        })
...
def import_master_cr_view(request):
    if request.method == "POST":
        form = ExcelUploadForm(request.POST, request.FILES)
        if form.is_valid():
            uploaded_file = request.FILES["excel_file"]
            df = pd.read_excel(uploaded_file, sheet_name="Planning_Sheet")
            ...
            return redirect("import-master-cr")

    else:
        form = ExcelUploadForm()

    return render(request, "import_master_cr.html", {"form": form})